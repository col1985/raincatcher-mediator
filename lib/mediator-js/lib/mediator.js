...
(function(global, factory) {
  ...
  var monitoring = require('./../../../initMonitoring.js');
  var stats = monitoring.stats;
  var logger = monitoring.log;
  ...
  Channel.prototype.addSubscriber = function(fn, options, context, statsTimer) {
  ...
  Channel.prototype.publish = function(data, statsTimer) {
    var x = 0,
        y = this._subscribers.length,
        shouldCall = false,
        subscriber, l,
    ...
    for(x, y; x < y; x++) {
      ...
      if (shouldCall) {
        ...
        logger('publish: ', {
          namespace: data[1].namespace,
          topicUid: data[0].topicUid
        });
  ...
  Mediator.prototype.subscribe = function(channelName, fn, options, context, statslabel) {
    var channel = this.getChannel(channelName || "", false);
    var label  = statslabel && statslabel.length > 0 ? statslabel : 'mediator-subscribe';
    ...
    var timer = stats.time(label, data);

    logger('mediator-subscribe', channelName);

    return channel.addSubscriber(fn, options, context, timer);
  }
  ...
  Mediator.prototype.once = function(channelName, fn, options, context) {
    ...
    return this.subscribe(channelName, fn, options, context, 'mediator-once');
  }
  ...
  Mediator.prototype.publish = function(channelName) {
    ...
    var timer = stats.time('mediator-publish', data);
    logger('mediator-publish', channelName);

    channel.publish(args, timer);
  }
